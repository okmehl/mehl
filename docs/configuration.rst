...
- ``IMAP_SERVICE``: the container that is running the IMAP server (default: ``imap``, port 143)
- ``LMTP_SERVICE``: the container that is running the LMTP server (default: ``imap:2525``)
- ``HOSTIMAP_SERVICE``: the container that is running the IMAP server for the webmail (default: ``imap``, port 10143)
- ``HOST_POP3``: the container that is running the POP3 server (default: ``imap``, port 110)
- ``SMTP_SERVICE``: the container that is running the SMTP server (default: ``smtp``, port 25)
- ``AUTHSMTP_SERVICE``: the container that is running the authenticated SMTP server for the webnmail (default: ``smtp``, port 10025)
- ``ADMIN_SERVICE``: the container that is running the admin interface (default: ``admin``)
- ``ANTISPAM_MILTER_SERVICE``: the container that is running the antispam milter service (default: ``antispam:11332``)
- ``ANTISPAM_WEBUI_SERVICE``: the container that is running the antispam webui service (default: ``antispam:11334``)
- ``ANTIVIRUS_SERVICE``: the container that is running the antivirus service (default: ``antivirus:3310``)
- ``WEBMAIL_SERVICE``: the container that is running the webmail (default: ``webmail``)
- ``WEBDAV_SERVICE``: the container that is running the webdav server (default: ``webdav:5232``)
- ``REDIS_SERVICE``: the container that is running the redis daemon (default: ``redis``)
- ``WEBMAIL_SERVICE``: the container that is running the webmail (default: ``webmail``)
