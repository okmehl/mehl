...
def run(debug):
    try:
        fetches = requests.get("http://" + os.environ.get("ADMIN_SERVICE", "admin") + "/internal/fetch").json()
        smtphost, smtpport = extract_host_port(os.environ.get("SMTP_SERVICE", "smtp"), None)
        if smtpport is None:
        ...
        for fetch in fetches:
            ...
            try:
                ...
                requests.post("http://" + os.environ.get("ADMIN_SERVICE", "admin") + "/internal/fetch/{}".format(fetch["id"]),
