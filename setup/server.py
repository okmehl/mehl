...
def build_app(path):
    ...
    prefix_bp.jinja_loader = jinja2.ChoiceLoader([
        jinja2.FileSystemLoader(os.path.join(path, "configs")),
    ...
    @prefix_bp.route("/configs/<uid>", methods=["GET"])
    @root_bp.route("/configs/<uid>", methods=["GET"])
    def configs(uid):
        data = json.loads(db.get(uid))
        config_zip_file = BytesIO()

        with ZipFile(config_zip_file, 'w') as zipFile:
            for folderName, subfolders, filenames in os.walk(os.path.join(path, 'configs')):
                for filename in filenames:
                    content = render_config(os.path.basename(folderName), filename, data)
                    zipFile.writestr(os.path.basename(folderName) + '/' + filename, content)

        config_zip_file.seek(0)

        return flask.send_file(config_zip_file, attachment_filename='configs.zip', as_attachment=True)
